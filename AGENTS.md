...
`web/static/js/editor.js` is generated by esbuild from `web/js-src/editor.mjs`. After editing the source, rebuild the bundle and the Go binary (which re-embeds it):
...
`@codemirror/state` and `@codemirror/view` MUST stay deduplicated to a single instance in `node_modules` — `package.json` pins compatible ranges and uses `overrides` for that. If you bump CodeMirror packages, re-check that only one copy of each ships in the bundle, or `yCollab` will throw "Unrecognized extension value" / "multiple instances of @codemirror/state" at runtime and the editor will not render.

## Manual e2e

No scripted browser harness. To validate collaborative editing, open the same `/d/<ULID>` URL in two browser tabs and type in each; edits must propagate live and survive a reload. In Cursor, drive this through the `cursor-ide-browser` MCP (`browser_navigate`, `browser_click`, `browser_type`, `browser_snapshot`).
